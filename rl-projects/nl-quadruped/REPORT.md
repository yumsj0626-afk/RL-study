...
**속도추종(velocity-tracking) 정책**이다. 직접 학습시켜 체크포인트
(`model_499.pt`)를 만들고 이후 freeze해 사용했다.

**MDP 구성 (실제 실행 로그 기준):**

- **관측 (48차원)**: `base_lin_vel(3)` · `base_ang_vel(3)` ·
  `projected_gravity(3)` · **`velocity_commands(3)`** · `joint_pos(12)` ·
  `joint_vel(12)` · `last_actions(12)`
  → 정책은 자기 상태 + **명령받은 목표 속도**를 함께 본다. 이 `velocity_commands`가 통합 지점이다.
- **행동 (12차원)**: 12개 다리 관절(FL/FR/RL/RR × hip/thigh/calf)의 위치 목표값.
- **보상 (가중치)**: 명령 추종 보상 `track_lin_vel_xy_exp(+1.5)`,
  `track_ang_vel_z_exp(+0.75)`을
  중심으로, 안정·효율 패널티 `lin_vel_z_l2(-2.0)`, `flat_orientation_l2(-2.5)`,
  `action_rate_l2(-0.01)`, `dof_torques_l2`, `dof_acc_l2` 등 10개 항목.
  `feet_air_time(+0.25)`로
  자연스러운 보행(발 들기)을 유도.
...
- **플래너**: spec → 점유 격자 → 8방향 A* → line-of-sight 단순화로
  waypoint. `forbidden`은 로봇 반경+
...
- **hard vs soft 구분**: "절대 피해"·"벽 붙지 마"는
  `forbidden_regions`(+clearance 0.3~0.5)로(T02·T10),
...
**(1) 하드 장애물 회피** — `center_block`
([preview](results/preview_center_block.png)): 중앙
[3,3]~[5,5] 금지 박스를 (로봇 반경+clearance만큼 팽창시킨 뒤) 좌상단으로 우회. waypoint 3개로
효율적 경로, 최종 궤적의 forbidden 진입 **0회**.

**(2) 소프트 의도의 실효성** — `safe_detour`
([preview](results/preview_safe_detour.png)):
...
**(3) 통제된 실패 5종** — `results/failure_taxonomy.md`,
`preview_fail_*.png`. 목표를 지어내지 않고
계획 단계에서 안전하게 멈추며, 각 케이스의 메시지를 분리 기록:

| 실패 케이스 | 메시지 |
| --- | --- |
| underspecified | Goal is underspecified (goal=null) |
| fail_goal_in_obstacle | Goal inside an inflated forbidden region |
| fail_start_in_obstacle | Start inside an inflated forbidden region |
...
![Go2가 자연어 명령을 수행하고 목표에 도착한 순간](results/isaac_arrival.png)
...
- 산출물: `nl_parser.py`·`planner.py`·`controller.py`·`preview_2d.py`·
  `run_experiments.py`·
...
- 인프라는 직접 Terraform 작성 대신 **NVIDIA 공식 Isaac Automator** 채택
  (내부적으로 Terraform+Ansible) —
...
- `./stop`/`./destroy`로 작업할 때만 과금(총 GPU 비용 수십 달러 규모).
  절차는 `docs/01_phase1_isaac_automator.md`.

### Phase 2 — Go2 보행 정책 학습 (제어 계층)

- Isaac Lab 스톡 태스크 `Isaac-Velocity-Flat-Unitree-Go2-v0`를
  **rsl_rl PPO로 직접 학습**(체크포인트 `model_499.pt`).
- `play --video`로 보행 + 헤드리스 녹화 검증. 이 과정에서 정책 인터페이스(관측 48 / 행동 12 / 보상 10항목,
  명령 = `velocity_commands` 3차원)를 파악해 **Phase 3 통합 지점을 확정**(§3).
- 스톡 태스크·기본 config를 그대로 써서 버전 호환 디버깅을 회피.
  절차는 `docs/02_phase2_go2_locomotion.md`.

![Isaac Lab에서 병렬 학습 중인 Go2 보행 정책](results/go2_training.png)

### Phase 3 — 자연어 → 정책 통합

- `isaac/nlq_play.py`(공식 `play.py` 복제 + NLQ 훅)로 NL → go-to-goal →
  velocity command를 정책에 주입(§5).
- 통합 디버깅 3종 해결: ① 명령 주입 위치(command manager 버퍼)
  ② 헤딩 추출 버그(공식 `euler_xyz_from_quat`)
...
- **컴퓨팅**: AWS EC2 `g5.2xlarge` (NVIDIA A10G 24GB), us-east-1.
  NVIDIA 공식 **Isaac Automator**로
  배포(내부 Terraform+Ansible). `./stop`/`./destroy`로 비용 통제(총 GPU 비용 수십 달러 규모).
- **재현 절차**: `docs/00_setup_and_aws.md`(자격증명·비용),
  `docs/01_phase1_isaac_automator.md`(배포),
